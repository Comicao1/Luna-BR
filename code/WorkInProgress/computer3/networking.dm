...
/obj/item/part/computer/networking/radio
	...
	init()
		..()
		spawn(5)
			radio_connection = radio_controller.add_object(src, src.frequency, src.d_filter)

	proc/set_frequency(new_frequency)
		if(radio_controller)
			radio_controller.remove_object(src, frequency)
			frequency = new_frequency
			radio_connection = radio_controller.add_object(src, frequency, d_filter)
		else
			frequency = new_frequency
			spawn(rand(5,10))
	...
	proc/post_signal(var/datum/signal/signal)
		...
		radio_connection.post_signal(src,signal,d_filter,range)
